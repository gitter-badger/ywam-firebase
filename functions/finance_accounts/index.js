...
exports.processIncome = functions.database.ref('/finance_accounts/{accountId}/income_transactions/{transactionId}').onWrite(event => {
   ...
   if(tran.payer_meta){
       ...
       p[p.length] =  new Promise(function(resolve, reject) {
        ...
        admin.database().ref('/finance_accounts/'+event.params.accountId+ '/settings/hide_keywords_income').once('value').then(snap => {
...
exports.updateIncomeCount = functions.database.ref('/finance_accounts/{accountId}/income_transactions').onWrite(event => {
      ...
      var updates ={unbookedIncomeCount:unbookedCount,
                    unbookedIncomeTotal:unbookedTotal}

         return     event.data.adminRef.parent.child('balances').update(updates)

})

exports.updateExpenseCount = functions.database.ref('/finance_accounts/{accountId}/expense_transactions').onWrite(event => {
...
exports.processExpense = functions.database.ref('/finance_accounts/{accountId}/expense_transactions/{transactionId}').onWrite(event => {
